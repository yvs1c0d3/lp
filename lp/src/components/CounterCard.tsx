...
export const CounterCard: React.FC<CounterCardProps> = ({
  ...
  // Map colors to their hover variants explicitly so Tailwind scanner picks them up
  // or use opacity for hover effect which works with any color
  const getButtonColorClass = (colorClass: string) => {
    // We just return the base color. We handle hover by opacity modification below 
    // to avoid constructing class names like 'hover:bg-blue-600' which Tailwind might miss.
    return colorClass;
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex flex-col justify-between h-full relative overflow-hidden">
      ...
      <div className="flex items-center justify-between mt-auto">
        ...
        <button
          onClick={onIncrement}
          className={`w-12 h-12 flex items-center justify-center rounded-xl transition-all active:scale-95 text-white shadow-md hover:brightness-90 ${color}`}
